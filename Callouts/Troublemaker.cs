...
public class Troublemaker : Callout
{
    ...
    private string[] _pedList = new string[] { "s_m_y_dealer_01", "u_m_m_jesus_01", "u_m_y_militarybum", "u_m_y_proldriver_01", "a_m_o_soucent_03", "u_m_o_tramp_01",
    ...
    public override void Process()
    {
        GameFiber.StartNew(delegate
        {
            if (_subject.DistanceTo(MainPlayer) < 20f)
            {
                ...
                if (_attack == false && _subject.DistanceTo(MainPlayer) < 6f && MainPlayer.IsOnFoot && _alreadySubtitleIntrod == false)
                {
                ...
                if (_subject.DistanceTo(MainPlayer) < 5f && MainPlayer.IsOnFoot && _alreadySubtitleIntrod == false)
                {
                    _subject.Face(MainPlayer);
                }
