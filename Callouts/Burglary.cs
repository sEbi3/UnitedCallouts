    ...
    public class Burglary : Callout
    {
        ...
        public override void Process()
        {
            GameFiber.StartNew(delegate
            {
                ...
                if (_Aggressor.Exists() && _Aggressor.IsDead) { Game.DisplayHelp("~y~Dispatch:~w~ Make sure no one else is in the apartment. Otherwise ~g~End~w~ the callout.", 5000); }
                if (_Aggressor.Exists() && Functions.IsPedArrested(_Aggressor)) { Game.DisplayHelp("~y~Dispatch:~w~ Make sure no one else is in the apartment. Otherwise ~g~End~w~ the callout.", 5000); }
