...
{
    [CalloutInfo("CarTrade", CalloutProbability.Medium)]
    public class CarTrade : Callout
    {
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            ...
            {
                case 1:
                    this.CalloutMessage = "~b~Dispatch:~s~ Illegal police car trade";
                    _callOutMessage = 1;
                    break;
                case 2:
                    this.CalloutMessage = "~b~Dispatch:~s~ Illegal police car trade";
        ...
        public override void OnCalloutNotAccepted()
        {
            ...
            if (_Buyer.Exists()) _Buyer.Delete();
            if (_Blip.Exists()) _Blip.Delete();
            if (_Seller.Exists()) _Seller.Delete();
        ...
        public override void Process()
        {
            GameFiber.StartNew(delegate
            {
                ...
                if (_Seller.IsDead && _Buyer.IsDead) End();
                if (Functions.IsPedArrested(_Seller) && Functions.IsPedArrested(_Buyer)) End();
            }, "CarTrade [UnitedCallouts]");
            base.Process();
        }
        public override void End()
        {
            if (_Seller.Exists()) _Seller.Dismiss();
            if (_Blip.Exists()) _Blip.Delete();
            if (_Buyer.Exists()) _Buyer.Dismiss();
