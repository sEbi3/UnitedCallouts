...
[CalloutInfo("[UC] Jewelry Robbery In Progress", CalloutProbability.Medium)]
public class JewelryRobbery : Callout
{
    ...
    public override bool OnBeforeCalloutDisplayed()
    {
        ...
        CalloutMessage = "Reports of a Jewelry Robbery in Progress";
        CalloutAdvisory = "Reports received of an ongoing jewelry robbery involving multiple masked suspects.";
    ...
    public override bool OnCalloutAccepted()
    {
        ...
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Jewelry Robbery in Progress callout accepted.");
        }
        else { Settings.DetailedLogging = false; }

        Game.LogTrivial("UnitedCallouts Log: Jewelry Robbery callout accepted.");
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Jewelry Robbery in Progress",
            "~b~Dispatch: ~w~Respond to an ongoing jewelry robbery. Multiple masked suspects reported. Use extreme caution, backup is en route. Respond with ~r~Code 3~w~.");
        ...
        if (Settings.ActivateAiBackup)
        {
            ...
            if (Settings.DetailedLogging)
            {
                Game.LogTrivial("[UnitedCallouts LOG:] Jewelry Robbery in Progress Callout: AI Backup has been spawned.");
            }
            else { Settings.DetailedLogging = false; }
        }
        else
    ...
    {
        if (!_hasBegunAttacking && !_pursuitCreated && _spawnPoint.DistanceTo(MainPlayer) < 25f)
        {
            ...
            GameFiber.StartNew(() =>
            {
                ...
                else if (_scene3)
                {
                    ...
                    if (Settings.DetailedLogging)
                    {
                        Game.LogTrivial("[UnitedCallouts LOG:] Jewelry Robbery in Progress Callout: Pursuit has started.");
                    }
                    else { Settings.DetailedLogging = false; }
            ...
            }, "Jewelry Robbery in Progress [UnitedCallouts]");
        }
        if (MainPlayer.IsDead) End();
        if (Game.IsKeyDown(Settings.EndCall)) End();
        if (_a1 != null && _a1.IsDead && _a2 != null && _a2.IsDead && _a3 != null && _a3.IsDead) End();
        if (_a1 != null && Functions.IsPedArrested(_a1) && _a2 != null && Functions.IsPedArrested(_a2) && _a3 != null && Functions.IsPedArrested(_a3)) End();
        base.Process();
    }
    ...
    public override void End()
    {
        ...
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Jewelry Robbery in Progress", "~b~You: ~w~Dispatch we're code 4. Show me ~g~10-8.");
        Functions.PlayScannerAudio("ATTENTION_THIS_IS_DISPATCH_HIGH ALL_UNITS_CODE4 NO_FURTHER_UNITS_REQUIRED");
        if (Functions.IsPursuitStillRunning(_pursuit)) { Functions.ForceEndPursuit(_pursuit); }
        if (Settings.DetailedLogging)
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Jewelry Robbery in Progress callout ended.");
        }
        else { Settings.DetailedLogging = false; }
        base.End();
    }
