...
[CalloutInfo("[UC] Drug Transaction In Progress", CalloutProbability.Medium)]
public class DrugTransaction : Callout
{
    ...
    public override bool OnBeforeCalloutDisplayed()
    {
        ...
        CalloutMessage = "Reports of a Drug Transaction in Progress";
        CalloutAdvisory = "Suspected narcotics transaction in progress. Use caution. No weapons reported at this time.";
    ...
    public override bool OnCalloutAccepted()
    {
        ...
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Drug Transaction In Progress callout accepted.");
        }
        else { Settings.DetailedLogging = false; }
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Drug Transaction In Progress",
            "~b~Dispatch: ~w~Respond to a reported drug transaction in progress. Respond with ~y~Code 2");
    ...
    {
        if (_dealer != null && _dealer.Exists() && _dealer.DistanceTo(MainPlayer.GetOffsetPosition(Vector3.RelativeFront)) < 75f && !_isArmed)
        {
        ...
        if (!_hasBegunAttacking && _dealer != null && _dealer.Exists() && _dealer.DistanceTo(MainPlayer.GetOffsetPosition(Vector3.RelativeFront)) < 60f)
        {
            ...
            GameFiber.StartNew(() =>
            {
                switch (_scenario)
                {
                    case > 40:
                        ...
                        Game.DisplayHelp("~b~Dispatch: ~w~The suspected buyer has raised his hands and is complying. Proceed with arrest.", 5000);
                    ...
                    default:
                        if (!_hasPursuitBegun)
                        {
                            ...
                            if (Settings.DetailedLogging)
                            {
                                Game.LogTrivial("[UnitedCallouts LOG:] Drug Transaction In Progress Callout: Pursuit has started.");
                            }
                            else { Settings.DetailedLogging = false; }
            ...
            }, "Drug Transaction In Progress [UnitedCallouts]");
        }
    ...
    public override void End()
    {
        ...
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Drug Transaction In Progress", "~b~You: ~w~Dispatch we're code 4. Show me ~g~10-8.");
        Functions.PlayScannerAudio("ATTENTION_THIS_IS_DISPATCH_HIGH ALL_UNITS_CODE4 NO_FURTHER_UNITS_REQUIRED");
        if (Functions.IsPursuitStillRunning(_pursuit)) { Functions.ForceEndPursuit(_pursuit); }
        if (Settings.DetailedLogging)
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Drug Transaction in Progress callout ended.");
        } else { Settings.DetailedLogging = false; }
        base.End();
    }
}