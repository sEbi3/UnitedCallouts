...
{
    [CalloutInfo("ArmoredPerson", CalloutProbability.Medium)]
    public class ArmoredPerson : Callout
    {
        private string[] wepList = new string[] { "WEAPON_MINIGUN", "WEAPON_MG", "WEAPON_COMBATMG" };
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            ...
            CalloutMessage = "~b~Dispatch:~s~ Reports of an armored person.";
            ...
            GameFiber.StartNew(delegate
            {
                ...
                if (_subject.Exists() && _subject.DistanceTo(Game.LocalPlayer.Character.GetOffsetPosition(Vector3.RelativeFront)) < 50f && !_hasBegunAttacking)
                {
                    if (_scenario > 40)
                    {
                        ...
                        Game.DisplayNotification("~b~Dispatch: ~w~The armored person is ~r~shooting~w~ on people!");
                        GameFiber.Wait(2000);
