...
{
    [CalloutInfo("Bicycle on the Freeway", CalloutProbability.Medium)]
    public class Bicycle : Callout
    {
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            ...
            CalloutMessage = "~b~Dispatch:~s~ Bicycle on the Freeway";
            ...
            GameFiber.StartNew(delegate
            {
                if (_subject.DistanceTo(Game.LocalPlayer.Character) < 20f)
                {
                    if (_IsStolen == true && _startedPursuit == false)
                    {
                        ...
                        _Bike.IsStolen = true;
                        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts Computer", "~y~Dispatch Information", "The ~g~bicycle~w~ from the suspect is a ~o~" + _Bike.Model.Name + "~w~. The ~g~bicyclew~w~ was ~r~stolen~w~.");
                        GameFiber.Wait(2000);
                    }
