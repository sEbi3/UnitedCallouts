...
public class SuspiciousAtmActivity : Callout
{
    ...
    public override bool OnBeforeCalloutDisplayed()
    {
        ...
        CalloutMessage = "Reports of Suspicious ATM Activity";
        CalloutAdvisory = "Reports received of suspicious activity at an ATM, with an individual observed carrying a crowbar.";
    ...
    public override bool OnCalloutAccepted()
    {
        ...
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Suspicious ATM Activity callout accepted.");
        }
        else { Settings.DetailedLogging = false; }
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Suspicious ATM Activity",
            "~b~Dispatch: ~w~Respond to suspicious activity at an ATM. Caller reports one individual carrying a crowbar. Use caution and advise upon arrival. Respond with ~r~Code 3");
    ...
    {
        if (_aggressor != null && _aggressor.Exists() && !_hasBegunAttacking && _aggressor.DistanceTo(MainPlayer.GetOffsetPosition(Vector3.RelativeFront)) < 20f)
        {
            switch (_scenario)
            {
                case > 40:
                ...
                default:
                {
                    if (!_hasPursuitBegun)
                    {
                        ...
                        _hasPursuitBegun = true;
                            if (Settings.DetailedLogging)
                            {
                                Game.LogTrivial("[UnitedCallouts LOG:] Suspicious ATM Activity Callout: Pursuit has started.");
                            } else { Settings.DetailedLogging = false; }
                     }
                    break;
                }
