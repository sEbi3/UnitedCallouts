...
class PublicPeaceDisturbance : Callout
{
    ...
    public override bool OnBeforeCalloutDisplayed()
    {
        ...
        CalloutMessage = "Public Peace Disturbance";
        CalloutAdvisory = "Reports received of a physical altercation between two individuals in a public area.";
    ...
    public override bool OnCalloutAccepted()
    {
        ...
        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            ...
            "~b~Dispatch: ~w~Respond to a fight in progress. Two individuals actively involved in a physical altercation. Use caution and advise upon arrival. Respond with ~r~Code 3");
        ...
        if (Settings.ActivateAiBackup)
        {
            ...
            if (Settings.DetailedLogging)
            {
                Game.LogTrivial("[UnitedCallouts LOG:] Public Peace Disturbance Callout: AI Backup has been spawned.");
            }
            else { Settings.DetailedLogging = false; }
        }
        else
    ...
    public override void Process()
    {
        if (!_hasBegunAttacking && MainPlayer.DistanceTo(_spawnPoint) < 100f)
        {
            ...
            if (Settings.DetailedLogging)
            {
                Game.LogTrivial("[UnitedCallouts LOG:] Public Peace Disturbance Callout: The fight started.");
            }
            else { Settings.DetailedLogging = false; }
        }
    ...
    public override void End()
    {
        ...
        if (Settings.DetailedLogging)
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Public Peace Disturbance callout ended.");
        }
        else { Settings.DetailedLogging = false; }
        base.End();
    }
