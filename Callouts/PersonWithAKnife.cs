...
public class PersonWithKnife : Callout
{
    ...
    public override bool OnBeforeCalloutDisplayed()
    {
        ...
        CalloutMessage = "Reports of a Person With a Knife";
        CalloutMessage = "Reports received of an individual carrying a knife in a public area.";
    ...
    public override bool OnCalloutAccepted()
    {
        ...
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Person With a Knife callout accepted.");
        }
        else { Settings.DetailedLogging = false; }

        Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts",
            "~y~Person With a Knife", "~b~Dispatch: ~w~Respond to reports of a person armed with a knife. Suspect described as one individual, weapon visible. Use extreme caution and advise upon arrival. Respond with ~r~Code 3");
    ...
    public override void Process()
    {
        ...
        if (!_hasBegunAttacking && _subject != null && _subject.Exists() && _subject.DistanceTo(MainPlayer.GetOffsetPosition(Vector3.RelativeFront)) < 18f)
            {
            if (Settings.DetailedLogging)
            {
                Game.LogTrivial("[UnitedCallouts LOG:] Reports of a Person With a Knife: Player arrived on scene.");
            }
            else { Settings.DetailedLogging = false; }
            _hasBegunAttacking = true;
                GameFiber.StartNew(() =>
                {
                    switch (_scenario)
                    {
                        case > 40:
                            ...
                            {
                                case 1:
                                    Game.DisplaySubtitle("~r~Suspect: ~w~Stay back! I don’t want anyone to come closer!", 4000);
                                    _hasSpoke = true;
                                    break;
                                case 2:
                                    Game.DisplaySubtitle(
                                        "~r~Suspect: ~w~Leave me alone, I’m not trying to hurt anyone!", 4000);
                                    _hasSpoke = true;
                                    break;
                                case 3:
                                    Game.DisplaySubtitle("~r~Suspect: ~w~Don’t come near me, I just want people to keep their distance!", 4000);
                                    _hasSpoke = true;
                                    break;
                            }
                ...
                }, "Reports of a Person With a Knife [UnitedCallouts]");
            }
    ...
    public override void End()
    {
        ...
        if (Settings.DetailedLogging)
        {
            Game.LogTrivial("[UnitedCallouts LOG:] Person With a Knife callout ended.");
        }
        else { Settings.DetailedLogging = false; }
        base.End();
    }
