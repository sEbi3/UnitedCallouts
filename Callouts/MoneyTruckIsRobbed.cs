...
{
    [CalloutInfo("MoneyTruck is Robbed", CalloutProbability.Medium)]
    class MoneyTruckIsRobbed : Callout
    {
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            ...
            this.CalloutMessage = "MoneyTruck is Robbed";
            this.CalloutPosition = _vehicleSpawnPoint;
        ...
        public override void OnCalloutNotAccepted()
        {
            ...
            if (_Aggressor2.Exists()) _Aggressor2.Delete();
            if (_Aggressor3.Exists()) _Aggressor3.Delete();
            if (_Aggressor4.Exists()) _Aggressor4.Delete();
            if (_stockade.Exists()) _stockade.Delete();
        ...
        {
            if (_Aggressor1.IsDead)
            {
                _Blip1.Delete();
            }
            if (_Aggressor2.IsDead)
            {
                _Blip2.Delete();
            }
            if (_Aggressor3.IsDead)
            {
                _Blip3.Delete();
            }
            if (_Aggressor4.IsDead)
            {
                _Blip4.Delete();
            }
            if (Functions.IsPedArrested(_Aggressor1) && Functions.IsPedArrested(_Aggressor2) && Functions.IsPedArrested(_Aggressor3) && Functions.IsPedArrested(_Aggressor4)) End();
            if (Game.LocalPlayer.Character.Position.DistanceTo(_stockade) < 40f)
            {
                if (_stockade.DistanceTo(Game.LocalPlayer.Character) < 200f)
                {
                    if (_Blip1.Exists()) _Blip1.Delete();
                    if (_Blip2.Exists()) _Blip2.Delete();
                    if (_Blip3.Exists()) _Blip3.Delete();
            ...
            if (_Aggressor1.IsDead && _Aggressor2.IsDead && _Aggressor3.IsDead && _Aggressor4.IsDead) End();
        }
        ...
        public override void End()
        {
            ...
            if (_Blip3.Exists()) _Blip3.Delete();
            if (_Blip4.Exists()) _Blip4.Delete();
            if (_Aggressor1.Exists()) _Aggressor1.Dismiss();
            if (_Aggressor2.Exists()) _Aggressor2.Dismiss();
            if (_Aggressor3.Exists()) _Aggressor3.Dismiss();
            if (_Aggressor4.Exists()) _Aggressor4.Dismiss();
            if (_stockade.Exists()) _stockade.Dismiss();
            Game.DisplayNotification("web_lossantospolicedept", "web_lossantospolicedept", "~w~UnitedCallouts", "~y~MoneyTruck is robbed", "~b~You: ~w~Dispatch we're code 4. Show me ~g~10-8.");
