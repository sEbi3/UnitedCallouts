...
namespace UnitedCallouts.Callouts
{
    ...
    public class ATMActivity : Callout
    {
        ...
        public override void Process()
        {
            ...
            {
                if (_Aggressor.Exists() && _Aggressor.DistanceTo(Game.LocalPlayer.Character.GetOffsetPosition(Vector3.RelativeFront)) < 20f && !_hasBegunAttacking)
                {
                    if (_scenario > 40)
                    {
                ...
                if (_Aggressor.Exists() && _Aggressor.IsDead) End();
                if (_Aggressor.Exists() && Functions.IsPedArrested(_Aggressor)) End();
